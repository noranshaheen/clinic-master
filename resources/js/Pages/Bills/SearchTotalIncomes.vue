    ...
    <app-layout>
        <div class="py-0">
            <div class="mx-auto sm:px-6 lg:px-8">
                <h2 class="py-3 px-1">{{ __("Incomes") }}</h2>
                <div class="bg-white shadow-xl sm:rounded-lg px-4 pb-4 pt-4">
                  ...
                  {{ __("Search Expenses") }}
                        ...
                        <jet-button @click="onExportIncomes()">
                            {{ __("Export") }}
                        </jet-button>
                    </div>
                </div>
                        ...
                        <tbody class="text-center border border-[#d4d4d4]">
                            <tr class="border border-[#d4d4d4]">
                                <td class="border border-[#d4d4d4]">
                                    {{ incomes[0].clinic.name }}
                                </td>
                                <td class="p-2 border border-[#d4d4d4]">
                                    {{ incomes[0].doctor.name }}
                                </td>
                                <td class="border border-[#d4d4d4]">
                                    {{ incomes.length }}
                                </td>
                                <td class="border border-[#d4d4d4]">
                                    {{ totalIncomesRequired(incomes) }}
                                </td>
                                <td class="border border-[#d4d4d4]">
                                    {{ totalIncomes() }}
                                </td>
...
// import VueExcelEditor from "vue3-excel-editor/src/VueExcelEditor.vue";
// import VueExcelColumn from "vue3-excel-editor/src/VueExcelColumn.vue";
...
export default {
    ...
    methods: {
        ...
        onSearchIncomes: function () {
            axios
                .post(route("bills.income.searchData.total"), this.form)
                .then((response) => {
                    this.expenses = []
                    this.items = []
        ...
        onExportIncomes() {
            ...
            axios({
                ...
                link.href = url;
                link.setAttribute("download", "IncomesReport.xlsx");
        ...
        totalIncomesRequired(incomes){
            ...
            if (incomes.length > 0) {
                incomes.forEach((el) => {
                    total += Number(el.appointment.amount);
                    if (el.appointment.fees !== null) {
                        var fee = el.appointment.fees;
                        fee.forEach((el) => {
