      ...
      <form @submit.prevent="submit">
        <div class="grid grid-cols-1 gap-4">
          <div>
            ...
            <div class="mt-4">
              <jet-label for="type" :value='__("X-Ray Name")' />
              <jet-input id="type" type="text" class="mt-1 block w-full" v-model="form.name" required />
            </div>

            <div class="mt-4">
              <jet-label for="type" :value='__("X-Ray Description")' />
              <jet-input id="type" type="text" class="mt-1 block w-full" v-model="form.description" :placeholder='__("(optional)")' />
            </div>
...
export default {
  ...
  methods: {
    ...
    SaveNewCustomer() {
      axios
        .post(route("xray.store"), this.form)
        .then((response) => {
          this.$store.dispatch("setSuccessFlashMessage", true);
