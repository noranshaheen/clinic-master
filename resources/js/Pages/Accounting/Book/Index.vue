                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4 mt-4">
                    <Table :resource="items">
                        <template #cell(actions)="{ item: item }">
                            ...
                            <jet-button class="me-2 mt-2">
                                <a :href="route('accounting.book.item.index', item.id)">
                                    {{ __("Book Entries") }}
                                </a>
                            </jet-button>
                        </template>
...
import EditItem from "@/Pages/Accounting/Book/Edit.vue";
...
export default {
    ...
    methods: {
        ...
        toggleItemStatus(item) {
            ...
            axios
                .post(route("accounting.book.store"), this.currentItem)
                .then((response) => {
        ...
        deleteItem(item) {
            swal({
                ...
                if (willApprove) {
                    this.currentItem = item;
                    axios
                        .post(route("accounting.book.delete"), this.currentItem)
                        .then((response) => {
                            this.$store.dispatch("setSuccessFlashMessage", true);
                            setTimeout(() => {
