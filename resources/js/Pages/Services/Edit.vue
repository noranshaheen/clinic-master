      ...
      <form @submit.prevent="submit">
        <div class="grid grid-cols-1 gap-4">
          <div>
            ...
            <div class="mt-4">
              <jet-label for="type" :value='__("Service Name")' />
              <jet-input id="type" type="text" class="mt-1 block w-full" v-model="form.name" required />
            </div>

            <div class="mt-4">
              <jet-label for="type" :value='__("Service Default Price")' />
              <jet-input id="type" type="text" class="mt-1 block w-full" v-model="form.price" required />
            </div>
...
export default {
  ...
  methods: {
    ...
    SaveNewCustomer() {
      axios
        .post(route("services.store"), this.form)
        .then((response) => {
          this.$store.dispatch("setSuccessFlashMessage", true);
