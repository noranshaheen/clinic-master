            ...
            <div class="mx-auto sm:px-6 lg:px-8">
                <div class="wrapper Gbg-white overflow-hidden shadow-xl sm:rounded-lg p-4 flex-col">
                    <!-- <div class="flex justify-end">
                        <JetButton class="-mb-8" @click="newInMovement()">
                            {{ __("Add Items To Stock") }}
                        </JetButton>
                    </div> -->
                    <Table :resource="in_movements">

                        <template #cell(clinic)="{ item: in_movement }">
                            {{ in_movement.stock.inventory.clinic.name }}
                        </template>
                        <template #cell(warehouse)="{ item: in_movement }">
                            {{ in_movement.stock.inventory.name + "/" + in_movement.stock.inventory.location }}
                        </template>
                        <template #cell(stock)="{ item: in_movement }">
                            {{ in_movement.stock.name }}
                        </template>
                        <template #cell(item)="{ item: in_movement }">
                            {{ in_movement.item.name }}
                        </template>
                        <template #cell(type)="{ item: in_movement }">
                            {{ in_movement.type == 'in' ? __("Inbound") : "" }}
                        </template>
...
export default {
    ...
    methods: {
        newInMovement(){
            this.$inertia.visit(route("inventory.create.ins"));
        },
        editCustomer(cust) {
            this.inventory = cust;
